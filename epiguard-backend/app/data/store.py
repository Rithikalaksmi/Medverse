"""
Central in-memory data store for EpiGuard.
This mirrors src/data/mockData.ts and is the single source of truth
for all API endpoints. Swap these lists with real DB queries later.
"""
...
GLOBAL_DISEASE_DATA = [
    ...
    {"countryCode": "CN", "countryName": "China",           "lat": 35.86,  "lng": 104.20,  "totalCases": 5600000, "activeCases": 412000, "severity": "critical",  "weeklyChange": 35.2, "disease": "influenza"},
    {"countryCode": "JP", "countryName": "Japan",           "lat": 36.20,  "lng": 138.25,  "totalCases": 780000,  "activeCases": 58000,  "severity": "high",     "weeklyChange": 8.9,  "disease": "influenza"},
...
HOSPITALS = [
    ...
    {"id": "h7",  "name": "Guy's and St Thomas' NHS Foundation",     "countryCode": "GB", "city": "London",       "lat": 51.50,  "lng": -0.12,   "type": "NHS Trust Hospital",            "beds": 1810, "specialties": ["Cardiology","Renal","Cancer","Neurology"],                         "phone": "+44-20-7188-7188",   "status": "available"},
...
ALLOCATION_SCENARIOS = [
    {"id": "s1", "label": "Mumbai Surge — 60 critical patients",  "cityId": "mum", "disease": "influenza", "severity": "critical", "patientCount": 60},
    {"id": "s2", "label": "Delhi Outbreak — 30 severe patients",  "cityId": "del", "disease": "dengue",    "severity": "severe",   "patientCount": 30},
    {"id": "s3", "label": "NYC Wave — 45 moderate patients",      "cityId": "nyc", "disease": "COVID-19",  "severity": "moderate", "patientCount": 45},
]

# ─────────────────────────────────────────────
# GENERATORS (same logic as mockData.ts)
# ─────────────────────────────────────────────
def generate_timeline(weeks: int = 24):
    data = []
    now = datetime.utcnow()
    base_cases = 50000.0
    random.seed(42)
    for i in range(weeks, -1, -1):
        ...
        variation = 1 + (math.sin(i * 0.4) * 0.2 + (random.random() - 0.4) * 0.15)
        base_cases = max(10000.0, base_cases * variation)
        cases = int(base_cases)
        deaths = int(cases * 0.012)
        recovered = int(cases * 0.82)
        week_num = math.ceil(i / 4 + 1)
        data.append({
...
def generate_seird(days: int = 90):
    population = 1_000_000
    beta, sigma, gamma, delta = 0.35, 0.2, 0.1, 0.005
    S, E, I, R, D = population - 1000, 800.0, 200.0, 0.0, 0.0
    timeline = []
    for day in range(days):
        new_exposed  = (beta * S * I) / population
        new_infected = sigma * E
        new_recovered= gamma * I
        new_deceased = delta * I
        S = max(0.0, S - new_exposed)
        E = max(0.0, E + new_exposed - new_infected)
        I = max(0.0, I + new_infected - new_recovered - new_deceased)
        R += new_recovered
        D += new_deceased
        timeline.append({
            "day": day,
...
def generate_allocation_result(city_id: str, patient_count: int, severity: str):
    """Simple rule-based resource allocation."""
    facility = next((r for r in MEDICAL_RESOURCES if r["cityId"] == city_id), None)
    if not facility:
        return None

    multiplier = {"critical": 1.5, "severe": 1.2, "moderate": 1.0}.get(severity, 1.0)
    beds       = min(int(patient_count * multiplier),       facility["availableBeds"])
    icu        = min(int(patient_count * 0.3 * multiplier), facility["availableIcuBeds"])
    vents      = min(int(patient_count * 0.15 * multiplier),facility["availableVentilators"])
    doctors    = min(int(patient_count * 0.5),              facility["availableDoctors"])
    nurses     = min(int(patient_count * 2.0),              facility["availableNurses"])

    priority = "IMMEDIATE" if severity == "critical" else "URGENT" if severity == "severe" else "STANDARD"
    ...
    return {
        ...
        "bedsAllocated":      beds,
        "icuAllocated":       icu,
        "ventilatorsAllocated": vents,
        "doctorsAllocated":   doctors,
        "nursesAllocated":    nurses,
        "priority":           priority,
    }
